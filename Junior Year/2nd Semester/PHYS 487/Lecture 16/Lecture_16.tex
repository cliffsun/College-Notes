...
So far, only looked at one polarization along the $z$ axis. In general, we have an arbitrary polarization $\hat{p}$. Thus, only consider 

\begin{equation}
    |p|^2 \rightarrow \langle |p \cdot n | \rangle^2
\end{equation}

If both $\hat{p}$ and $\hat{n}$ are parallel, then $|p \cdot n |^2 = p^2$. Other combinations are zero. Therefore, 

\begin{equation}
    \langle |p \cdot n | \rangle^2 = \frac{1}{3}p^2
\end{equation}

This means that the rate is reduced by exactly $1/3$. 
