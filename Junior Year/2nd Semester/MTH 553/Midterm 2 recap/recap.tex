...
Heat equation change of variables:

\begin{equation}
    v(x,t) = e^{-bt/k}u(x - at/k, t/k)
\end{equation}

Transforms 

\begin{equation}
    u_t = k\triangle u + a \cdot \nabla u + bu \rightarrow v_{t'} = \triangle' v
\end{equation}

Where $t' = t/k$ and $x' = x - at/k$.

Energy of a heat equation:

\begin{equation}
    \epsilon(t) = \int_{\Omega}u^2 dx
\end{equation}

For lower order terms in wave equation. For 

\begin{equation}
    v_{tt} - c^2v_{xx} + \delta^2 v = 0, \quad w(x,y,t) = \cos(c^{-1}\delta y)v(x,t)
\end{equation}


\begin{equation}
    v_{tt} - c^2v_{xx} - \delta^2 v = 0, \quad w(x,y,t) = \cosh(c^{-1}\delta y)v(x,t)
\end{equation}
